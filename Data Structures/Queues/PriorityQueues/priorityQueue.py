...
class PriorityQueue:
    ...
    def enqueue(self, data, priorityLevel): # puts an item in the line (end of queue)

        item = Node(data, priorityLevel) # node object

        # treats the letters as numbers to compare priorities of the item going into the list against other data already in list
        # gets priority level for the item to go into list
        if item.priorityLevel == 'A':
            plNew = 1
        if item.priorityLevel == 'B':
            plNew = 2
        if item.priorityLevel == 'C':
            plNew = 3
        if item.priorityLevel == 'D':
            plNew = 4
        ...
        if len(self.items) == 0: # just add to end of list if queue is empty
            ...
            while (i >= 0):
                ...
                if self.items[i].priorityLevel == 'A':
                    pl = 1
                if self.items[i].priorityLevel == 'B':
                    pl = 2
                if self.items[i].priorityLevel == 'C':
                    pl = 3
                if self.items[i].priorityLevel == 'D':
                    pl = 4

                if plNew < pl: # compares the two

...
* Description : An app that demonstrates prim's algo to get the MST for a graph. Does this through a pretend USA road map.
...
if __name__ == '__main__':
    ...
    startNode = mapMatrix.prims_algo()

    # formatting output
    orderedCityList = []
    for i in range(1,10):
        for city in cities:
            if startNode[i] == city.iData:
                orderedCityList.append(city)
    for i in range(1, 10):
        print(orderedCityList[i-1].sData, "to", cities[i].sData, ":", mapMatrix.graph[i][startNode[i]], "miles")
